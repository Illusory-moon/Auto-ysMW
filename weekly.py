...
OFFICIAL = {
    ...
    "enter_box": START_GAME,   # 官服：前往大厅/开始游戏在同一位置
    "need_exit": True,         # 官服：需识别【退出奇域】并按 F
}
# B服：奇域编号 20333726902，名字「成就结算吃金币通关01」字多需往右扩；局内 W 4 秒即结算
BILI = {
    "name": "B服",
    "search_guid": "20333726902",
    "history_list": (1445, 107, 1700, 1033),   # 名字较长，识别框往右扩一点
    "history_text": ("成就结算吃金币通关01",),
    "walk": [("w", 4.0)],
    "use_movement": False,
    "enter_texts": (SOLO_TEXT,),   # 多人地图，进入按钮是「单人挑战」
    "enter_box": SOLO_BOX,         # 位置与官服开始按钮不同
    "need_exit": False,            # B服吃满金币无提示自动结算，无需识别【退出奇域】
}

# 当前服务器配置（run_weekly 里按 UID 设置）
_server = OFFICIAL
# ============================================================
...
def _resolve_walk():
    ...
    """
    plan = _server["walk"]
    if _server.get("use_movement", True):
        return [(key, _move(key, default)) for key, default in plan]
    return list(plan)
...
def _first_search_entry(stop_event):
    ...
    ops.type_text(_server["search_guid"])
    ops.wait(0.3, stop_event)
    ops.press_key("enter")
    ops.wait(0.5, stop_event)
    ...
    ent_texts = _server["enter_texts"]
    enter_box = _server["enter_box"]
...
def _delete_and_select_next(stop_event):
    ...
    del_box = ops.locate_text(_server["history_text"], DELETE_LIST, timeout=10, stop_event=stop_event)
    if del_box is None:
        if stop_event.is_set():
            return False
        logger.error(f"阶段失败：管理列表未找到目标关卡【{_server['history_text'][0]}】")
        return False
    del_cy = int((del_box[1] + del_box[3]) / 2)
    del_x = (CHECKBOX[0] + CHECKBOX[2]) // 2
    logger.info(f"定位到关卡【{_server['history_text'][0]}】，点击其选中框 ({del_x}, {del_cy + DELETE_ROW_OFFSET})")
    ...
    target = ops.locate_text(_server["history_text"], _server["history_list"], timeout=10, stop_event=stop_event)
    if target is None:
        if stop_event.is_set():
            return False
        logger.error(f"阶段失败：历史列表未找到目标奇域【{_server['history_text'][0]}】")
        return False
    logger.info(f"已定位历史奇域，点击选中 {target}")
    ops.click_box(target)
    ops.wait(0.5, stop_event)
    # 进入按钮：官服【开始游戏】在 START_GAME；B服【单人挑战】在 SOLO_BOX
    ops.click_box(_server["enter_box"])
    if stop_event.is_set():
        return False
...
def run_weekly(stop_event):
    ...
    _server = _detect_server(stop_event)
    logger.info(f"使用服务器配置：{_server['name']}")

    if not _first_search_entry(stop_event):
    ...
    for run_index in range(1, LOOP_TIMES + 1):
        ...
        if not _in_wonderland(stop_event):
            logger.error(f"周常中止：第 {run_index} 次未完成‘奇域 -> 大厅’")
            return
        if stop_event.is_set():
            return
        if run_index < LOOP_TIMES:
            if not _delete_and_select_next(stop_event):
